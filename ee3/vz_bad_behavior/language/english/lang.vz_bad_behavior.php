...
// Settings labels
'config'                => 'Bad Behavior Configuration',
...
'offsite_forms_desc'    => 'Usually only needed if you support OpenID or are running an API that accepts form data from other websites.',
'whitelisted_ips'       => 'Whitelisted IPs',
'whitelisted_ips_desc'  => 'Visitors from these IP addresses will always be allowed. One IP address per line.',
'whitelisted_urls'      => 'Whitelisted URIs',
'whitelisted_urls_desc' => 'Visits to these urls will always be allowed. One URI (or partial URI) per line. You may also enclose a pattern with tildes to perform regular expression matching, e.g. <code>~/blog/\\\d{4}(/\\\d{2})?$~i</code><br/><br/>Note that the trailing slash (if there is one) is stripped before matching.',

'rev_proxy'                    => 'Reverse Proxy',
'reverse_proxy'                => 'Enable Reverse Proxy?',
'reverse_proxy_desc'           => 'If you are using Bad Behavior behind a reverse proxy, load balancer, HTTP accelerator, content cache or similar technology, enable the Reverse Proxy option.',
'reverse_proxy_header'         => 'Reverse Proxy Header',
'reverse_proxy_header_desc'    => 'Your reverse proxy servers must set the IP address of the Internet client from which they received the request in an HTTP header. If you don\'t specify a header, X-Forwarded-For will be used. Most proxy servers already support X-Forwarded-For and you would then only need to ensure that it is enabled on your proxy servers. Some other header names in common use include X-Real-Ip (nginx) and Cf-Connecting-Ip (CloudFlare).',
'reverse_proxy_addresses'      => 'IP Addresses or CIDR Ranges of Proxy Servers',
'reverse_proxy_addresses_desc' => 'If you have a chain of two or more reverse proxies between your server and the public Internet, you must specify all of the IP address ranges (in CIDR format) of all of your proxy servers, load balancers, etc. Otherwise, Bad Behavior may be unable to determine the client\'s true IP address. One address per line.',
