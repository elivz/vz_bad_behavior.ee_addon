...
/**
 * EE3 Bad Behavior Extension
 *
 * @package     ExpressionEngine
 * @subpackage  Addons
 * @category    Extension
 * @author      Orig: Eli Van Zoeren, ilab
 */

class Ee3_bad_behavior_ext {
	...
	*/
	public function settings_form($settings) {
		...
		$global_settings = (array) ee()->config->item('ee3_bad_behavior');
	...
	*/
	public function bad_behavior($session) {
		...
		if ( isset($_GET['bb_logs']) && AJAX_REQUEST && stristr($_SERVER['HTTP_REFERER'], 'ee3_bad_behavior') ) {
			echo $this->_logs();
	...
	*/
	private function _logs() {
		ee()->load->library('table');
//		ee()->lang->loadfile('ee3_bad_behavior');
...
function bb2_read_settings() {
	...
	$default_settings = Ee3_bad_behavior_ext::$default_settings;
	...
	if (isset(ee()->extensions->extensions['sessions_start'])) {
		foreach(ee()->extensions->extensions['sessions_start'] as $priority => $extension) {
			if (isset($extension['Ee3_bad_behavior_ext'])) {
				// Retrieve the saved settings
				if ($extension['Ee3_bad_behavior_ext']['1'] != '') {
					$settings = unserialize($extension['Ee3_bad_behavior_ext']['1']);
...
/* End of file ext.ee3_bad_behavior.php */
/* Location: /system/user/addons/ee3_bad_behavior/ext.ee3_bad_behavior.php */